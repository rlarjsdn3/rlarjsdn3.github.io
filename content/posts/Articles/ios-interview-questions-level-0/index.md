...
> **답변**:
>
> 직렬 큐(Serial Queue)로 보내진 작업들은 모두 하나의 스레드에서 보내진 순서에 맞게 순차적으로 처리됩니다. 반면 동시 큐(Concurrent Queue)로 보내진 작업들은 보내진 순서와 상관없이 여러 스레드에서 동시에 처리됩니다. 그래서 어떤 작업이 먼저 끝나는지 알 수 없습니다.
...
> **답변**:
>
> 앱이 포그라운드에서 백그라운드로 전환되면 수행 중인 작업이 모두 가장 낮은 우선순위(background)로 변경됩니다. iOS는 사용자에게 직접 보여지는 
>
> 
